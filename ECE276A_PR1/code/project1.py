...
def plot_o(roll,est_roll,pitch,est_pitch,yaw,est_yaw,dataset_name):
    f,(fig1,fig2,fig3) = plt.subplots(3,1,figsize=(15,10))
    f.suptitle("Trainingset " + dataset_name)

    fig1.set_title("Roll")
    fig1.set_xlim(0,len(est_roll))
    fig1.plot(est_roll, label = 'Optimized Estimated roll')
    fig1.plot(roll,label = "Vicon ground truth")
    fig1.legend()

    fig2.set_title("Pitch")
    fig2.set_xlim(0,len(est_pitch))
    fig2.plot(est_pitch, label = 'Optimized Estimated pitch')
    fig2.plot(pitch,label = "Vicon ground truth")
    fig2.legend()

    fig3.set_title("Yaw")
    fig3.set_xlim(0,len(est_yaw))
    fig3.plot(est_yaw, label = 'Optimized Estimated yaw')
    fig3.plot(yaw,label = "Vicon ground truth")
    fig3.legend()
...
plot_o(roll,o_roll,pitch,o_pitch,yaw,o_yaw,dataset)
